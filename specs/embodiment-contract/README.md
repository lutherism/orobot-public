...
Every generalist robot-control policy in 2026 — [OpenVLA](https://arxiv.org/abs/2406.09246), [π0 / π0.5](https://www.pi.website/download/pi0.pdf), [Octo](https://arxiv.org/abs/2405.12213), [NVIDIA GR00T N1.x](https://arxiv.org/abs/2503.14734), [Gemini Robotics 1.5](https://arxiv.org/abs/2510.03342), [Figure Helix](https://www.figure.ai/news/helix) — consumes images and proprioception and emits actions in **some action space at some frequency for some embodiment**. None of them read URDF as direct input to the neural policy *network* at inference time. The URDF feeds the IK and simulator; the *policy* needs the action/state vector contract, the camera frame, the gripper convention, the control rate.

That contract is the layer every VLA implicitly requires and **none of them standardize**. The closest existing artifact is NVIDIA GR00T's `modality.json` — a key→slice manifest over a flat state vector with no joint limits, link lengths, kinematic tree, inertias, control frequency, camera intrinsics, or gripper convention. `embodiment.json` is strictly richer at the contract layer and stays compatible with that ecosystem.
...
The claim "no public 2026 generalist policy ingests URDF as direct input to the neural policy *network* at inference time" rests on a survey of the following models. URDF is used by surrounding runtime stacks for IK, simulation, joint-limit clamping, and synthetic-data generation — and may inform per-dataset action-normalization tables that rescale policy *outputs* — but in none of these models does the policy network itself read URDF as a tensor, parsed token, or structured prompt.

| Model | Inference inputs to the network | Embodiment conditioning | Citations | Verified |
|---|---|---|---|---|
| OpenVLA / OpenVLA-OFT | RGB image + NL instruction; OFT adds proprio | None at network input; `unnorm_key` string selects post-hoc action de-norm stats (data-driven, not URDF) | [arXiv 2406.09246](https://arxiv.org/abs/2406.09246), [openvla GitHub](https://github.com/openvla/openvla), [OpenVLA-OFT site](https://openvla-oft.github.io/) | High |
| RT-X / Open X-Embodiment | Images + language; 7-D EE-pose action across 22 embodiments | **Zero explicit conditioning** — embodiment inferred from visual context; per-dataset action de-norm outside the network | [arXiv 2310.08864](https://arxiv.org/abs/2310.08864) | High |
| Octo | RGB + (optional language or goal-image); readout tokens | New embodiments add a new lightweight head + normalization stats (not URDF) | [arXiv 2405.12213](https://arxiv.org/abs/2405.12213), [octo-models.github.io](https://octo-models.github.io/) | High |
| π0 / π0.5 (openpi) | Images dict + proprio `state` vector + tokenized prompt (see `openpi/src/openpi/models/model.py` `Observation` dataclass) | `action_dim` is a fixed trained-checkpoint property (7–32 across published configs; **not 18-D as some summaries claim**); per-embodiment adapter classes (`AlohaInputs`) hard-code joint sign flips in Python | [π0 PDF](https://www.pi.website/download/pi0.pdf), [π0.5 PDF](https://www.pi.website/download/pi05.pdf), [openpi GitHub](https://github.com/Physical-Intelligence/openpi) | High |
| NVIDIA GR00T N1.x | Images + language + proprio `float32` state vector + `--embodiment-tag` string (case-insensitive enum hand-mapped in Python) | URDF feeds Isaac Sim sim-twin authoring → synthetic data → per-embodiment MLP projector weights at training time, then discarded; N1.7's relative-EE-action space further abstracts away kinematics | [arXiv 2503.14734](https://arxiv.org/abs/2503.14734), [Isaac-GR00T GitHub](https://github.com/NVIDIA/Isaac-GR00T) | High (0.92) |
| Gemini Robotics ER (1.5/1.6) | Text + image + video + audio → points / boxes / plans (orchestration, not motor control) | Embodiment exposed as named tools (e.g., `robot_run_instruction`, `robot_open_gripper`) — capability surface, never a robot-description string | [arXiv 2510.03342](https://arxiv.org/abs/2510.03342), [Gemini Robotics overview](https://ai.google.dev/gemini-api/docs/robotics-overview), [gemini-robotics-sdk](https://github.com/google-deepmind/gemini-robotics-sdk) | High (0.90) |
| Gemini Robotics 1.5 VLA | Visual + language → motor commands (closed) | Motion Transfer mechanism is sold as *absence* of per-embodiment specialization; training-time data mixing, not runtime URDF | Same arXiv 2510.03342 | Medium-High (0.75) — paper body unread |
| Figure Helix | Monocular images + proprio (wrist pose + finger positions) + NL commands → wrist pose / finger / torso targets | Embodiment-locked to Figure 02 humanoid; no URDF mention | [Figure Helix announcement](https://www.figure.ai/news/helix) | Medium (0.65) — single source |
| X-VLA (ICLR 2026) | Multi-view images + language + proprio (joint pos + EE pose) → 20-D action vector | Soft prompts `pi ≈ Φ(hi)` are *randomly initialized and learned implicitly through training*, not URDF-derived; `meta.json` contains trajectory paths only | [arXiv 2510.10274](https://arxiv.org/abs/2510.10274), [X-VLA GitHub](https://github.com/2toinf/X-VLA) | High |
| ALOHA / Mobile ALOHA | Wrist + egocentric camera RGB + arm joint positions → 14-D / 16-D action | **Not a generalist policy** — hardware platform + data convention (50 Hz, fixed camera layout) + per-task ACT/Diffusion-Policy baselines; listed for reference, outside the claim's scope | [ALOHA PDF](https://tonyzhaozh.github.io/aloha/aloha.pdf), [Mobile ALOHA arXiv 2401.02117](https://arxiv.org/html/2401.02117v1) | High (reference, not policy) |

**Falsifier:** a single shipping or public-weights 2026 generalist policy whose inference call passes URDF (or any structured robot-description schema) as a tensor, parsed token, or structured prompt *into the policy network*. Action-normalization lookup tables, embodiment-tag enum strings, and learned per-embodiment projector heads selected at runtime are **not** URDF-as-network-input. If a counterexample exists or ships, the claim is wrong and this schema's positioning needs revision — please file an issue with the citation.

**Provenance:** the survey above was compiled via secondary research (May 2026) and then **re-verified by 5 independent LLM agents that read primary sources directly** — arXiv papers, model cards, GitHub source (including the `openpi` `Observation` dataclass, the Isaac-GR00T `modality.json` and `--embodiment-tag` machinery, the `gemini-robotics-sdk` `embodiments/` directory, X-VLA's soft-prompt definition). Per-model confidence ratings in the right column reflect the verification pass, not the original survey. Closed-weights models (Gemini VLA 1.5, Helix) carry necessarily lower confidence because internal architecture is hand-waved in public docs.
